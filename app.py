...
GROUP_1_COUNTS_INDICATORS = [
    ...
    'TLDel90Cnt24', 'TLDel60CntAll', 'TLBadDerogCnt', 'TLDel60Cnt24' # This list will also now align to 27 unique features
]
...
# This dummy data is now constructed to have exactly 27 numerical features,
# matching the adjusted NUMERICAL_FEATURES list.
dummy_data_for_training = pd.concat([
    pd.DataFrame({
        'ID': [f'Good_ID_{i}' for i in range(20)],
        'DerogCnt': [0]*20, 'CollectCnt': [0]*20, 'BanruptcyInd': [0]*20,
        'InqCnt06': [random.randint(0,2) for _ in range(20)],
        'InqTimeLast': [random.randint(80,150) for _ in range(20)],
        'InqFinanceCnt24': [random.randint(0,1) for _ in range(20)],
        'TLTimeFirst': [random.randint(180,300) for _ in range(20)],
        'TLTimeLast': [random.randint(20,50) for _ in range(20)],
        'TLCnt03': [random.randint(0,2) for _ in range(20)],
        'TLCnt12': [random.randint(2,5) for _ in range(20)],
        'TLCnt24': [random.randint(4,8) for _ in range(20)],
        'TLCnt': [random.randint(8,15) for _ in range(20)],
        'TLSum': [random.randint(40000,70000) for _ in range(20)],
        'TLMaxSum': [random.randint(10000,20000) for _ in range(20)],
        'TLSatCnt': [random.randint(7,12) for _ in range(20)],
        'TLDel60Cnt': [0]*20,
        'TLBadCnt24': [0]*20, 'TL75UtilCnt': [0]*20, 'TL50UtilCnt': [0]*20,
        'TLBalHCPct': [round(random.uniform(0.1,0.3),2) for _ in range(20)],
        'TLSatPct': [round(random.uniform(0.9,0.99),2) for _ in range(20)],
        'TLDel3060Cnt24': [0]*20,
        'TLDel90Cnt24': [0]*20, 'TLDel60CntAll': [0]*20,
    ...
    pd.DataFrame({ # Bad applicants (TARGET = 1) - high derogatory, low credit age, high inquiries
        'ID': [f'Bad_ID_{i}' for i in range(10)],
        'DerogCnt': [random.randint(1,3) for _ in range(10)],
        'CollectCnt': [random.randint(1,2) for _ in range(10)],
        'BanruptcyInd': [random.choice([0,1]) for _ in range(10)],
        'InqCnt06': [random.randint(3,6) for _ in range(10)],
        'InqTimeLast': [random.randint(5,30) for _ in range(10)],
        'InqFinanceCnt24': [random.randint(1,4) for _ in range(10)],
        'TLTimeFirst': [random.randint(30,80) for _ in range(10)],
        'TLTimeLast': [random.randint(2,15) for _ in range(10)],
        'TLCnt03': [random.randint(1,4) for _ in range(10)],
        'TLCnt12': [random.randint(5,10) for _ in range(10)],
        'TLCnt24': [random.randint(9,18) for _ in range(10)],
        'TLCnt': [random.randint(12,25) for _ in range(10)],
        'TLSum': [random.randint(15000,35000) for _ in range(10)],
        'TLMaxSum': [random.randint(3000,9000) for _ in range(10)],
        'TLSatCnt': [random.randint(0,5) for _ in range(10)],
        'TLDel60Cnt': [random.randint(1,3) for _ in range(10)],
        'TLBadCnt24': [random.randint(1,2) for _ in range(10)],
        'TL75UtilCnt': [random.randint(2,5) for _ in range(10)],
        'TL50UtilCnt': [random.randint(4,8) for _ in range(10)],
        'TLBalHCPct': [round(random.uniform(0.7,0.95),2) for _ in range(10)],
        'TLSatPct': [round(random.uniform(0.3,0.6),2) for _ in range(10)],
        'TLDel3060Cnt24': [random.randint(1,3) for _ in range(10)],
        'TLDel90Cnt24': [random.randint(1,2) for _ in range(10)],
        'TLDel60CntAll': [random.randint(1,4) for _ in range(10)],
...
def get_model_pipeline():
    ...
    try:
        ...
        st.sidebar.write(f"DEBUG: Columns used for dummy training: {dummy_features_for_training}")
        st.sidebar.write(f"DEBUG: Shape of dummy data for training: {dummy_data_for_training[dummy_features_for_training].shape}")
        ...
        if missing_dummy_features:
            st.error(f"Missing dummy features for model training: {missing_dummy_features}. Please check dummy_data_for_training.")
            raise ValueError("Missing dummy features for model training.") # Raise error to stop init
        ...
        raise # Re-raise to stop app if model init fails
    return model_pipeline

# Get the cached model pipeline
...
def get_credit_score_ml(df_input: pd.DataFrame, approval_threshold: int, actual_target_column: str): 
    ...
    st.write(f"DEBUG: Shape of data for prediction: {df_processed_for_prediction.shape}")
            ...
            if not actual_target_col_name:
                ...
                try:
                    with st.spinner(f"Downloading and analyzing '{file_to_analyze}' from S3..."):
                        ...
                        # Verify all NUMERICAL_FEATURES are present in df before prediction
                        missing_features_in_df = [f for f in NUMERICAL_FEATURES if f not in df.columns]
                        if missing_features_in_df:
                            st.error(f"Critical Error: The following expected numerical features are missing from your CSV after processing: {missing_features_in_df}. Please check your CSV file's column names.")
                            st.session_state['scored_data'] = pd.DataFrame()
                            st.rerun()
